...
function allMonstersDead(eim, player) {
	...
	if (priceRand <= 35) {
		...
		} while(ii.getSlotMax(player.getClient(), weaponId) == 0);
		...
		*/
		...
		price = client.Item(scrolls30[scrollRand], 0, 1);
		...
		price = new client.Item(powerUps[powerRand][0], 0, powerUps[powerRand][1]);
	} else if (priceRand > 70 && priceRand <= 75) {
		// throwing stars
		var starId = 2070003 + Math.floor(Math.random() * 14);
		price = new client.Item(starId, 0, ii.getSlotMax(player.getClient(), starId));
		...
		price = new client.Item(scrolls60[scrollRand], 0, 1);
	} else {
		// 40% scroll
		...
		price = new client.Item(scrolls40[scrollRand], 0, 1);		
	}
