				...
				if (mats instanceof Array) {
					for(var i = 0; complete && i < mats.length; i++)
					{
						if (matQty[i] == 1)	{
							...
							var count = 0;
							var iter = cm.getPlayer().getInventory(MapleInventoryType.ETC).listById(mats[i]).iterator();
							while (iter.hasNext()) {
					...
					var count = 0;
					var iter = cm.getPlayer().getInventory(MapleInventoryType.ETC).listById(mats).iterator();
					while (iter.hasNext()) {
						count += iter.next().getQuantity();
					}
					if (count < matQty)
			...
			if (!complete) 
				...
				if (stimulator){ //check for stimulator
					...
					if (deleted != 0)
					{
						...
						MapleInventoryManipulator.addFromDrop(cm.getC(), newItem, "Created " + item  + " at Sarah (2040020, map 220000303) using a stimulator");
