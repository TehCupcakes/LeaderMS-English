			...
			if (selection == 0)
			{
				if (cm.getChar().getGuildId() > 0) {
					if (cm.getPlayer().getGuildRank() == 1)
						cm.sendYesNo("Creating or changing Guild Emblem costs #b" + cm.getPlayer().emblemCost() + " mesos#k, are you sure you want to continue?");
					else
						cm.sendOk("You must be the Guild Leader to change the Emblem. Please tell your leader to speak with me.");
				} else {
					cm.sendOk("You must be a Guild Leader to change the Emblem. Please speak with Heracle to create a Guild, then talk to me.");
					cm.dispose();
				}
			}
