...
function action(mode, type, selection) {
    ...
    if (mode == -1) {//ExitChat
        ...
        if(cm.getPlayer().getMap().getId() == 922010000){
            ...
            if (status == 0) {
                cm.sendYesNo("You will need start from scratch if you want to try this mission after leaving this stage. Are you sure you want to leave this map?");
            }else if (status == 1) {
                cm.sendNext("All right, I'll take you out!");
            }else if (status == 2) {
                var eim = cm.getPlayer().getEventInstance(); 
                if(eim == null){
                    cm.sendOk("Wait, Hey! How did you get here?\r\nOh well, you can leave anyway.");
                }else{
                    if(isLeader() == true){
                        eim.disbandParty();
