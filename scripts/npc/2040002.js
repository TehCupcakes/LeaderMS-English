...
function action(mode, type, selection) {
	if(mode == -1) {
		...
		if(cm.getQuestStatus(3230).getId() == Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
			if(status == 0) {
				...
				if(entry == false) {
					...
					dh.getInstance("DollHouse").registerPlayer(cm.getPlayer());
				}
				cm.dispose();
			}
		} else if(cm.getQuestStatus(3230).getId() == Packages.client.MapleQuestStatus.Status.COMPLETED) {
