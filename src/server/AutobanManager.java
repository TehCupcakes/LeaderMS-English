...
public class AutobanManager implements Runnable {
	...
	public void autoban (MapleClient c, String reason) {
		if (c.getPlayer().isGM()) {
			System.out.println("[h4x] Trying to a/b gm - something fishy going on: {}" + reason);
		}
	...
	public synchronized void addPoints(MapleClient c, int points, long expiration, String reason) {
		...
		if (this.points.get(acc) >= AUTOBAN_POINTS) {
			...
			StringBuilder banReason = new StringBuilder("Autoban no personagem ");
			...
			if (c.getPlayer().isGM()) {
				System.out.println("[h4x] Trying to a/b gm - something fishy going on: {}" + banReason.toString());
			} else {
				c.getPlayer().ban(banReason.toString(), true);
				try {
					c.getChannelServer().getWorldInterface().broadcastMessage(null, 
						MaplePacketCreator.serverNotice(0, "[Autoban] " + name + " banido pelo sistema. (Ultima razao: " + reason + ")").getBytes());
				} catch (RemoteException e) {
					c.getChannelServer().reconnectWorld();
				}
				System.out.println("[h4x] Autobanned player " + name + " (accountid " + acc + ")");
			}
			return;
		}
		if (expiration > 0) {
