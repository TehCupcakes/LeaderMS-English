...
private static MapleStatEffect loadFromData(MapleData source, int sourceid, boolean skill, boolean overTime) {
	...
	private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
		if(skill && (this.sourceid == 4001003 || this.sourceid == 5101007) &&  applyfrom.getMap().getDisableInvincibilitySkills() && !applyfrom.isGM())
        {
            applyfrom.getClient().getSession().write(MaplePacketCreator.enableActions());
            applyfrom.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "Invincibility skills are disabled in this map."));
            return false;
        }
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
       ...
       if (localstatups.size() > 0) {
            if (isDash()) {
                ...
                applyto.getClient().getSession().write(MaplePacketCreator.showDashP(localstatups, localX, localY, seconds));
            } else if (isInfusion()) {
                applyto.getClient().getSession().write(MaplePacketCreator.giveInfusion(seconds, x));
            } else {
                applyto.getClient().getSession().write(MaplePacketCreator.giveBuff((skill ? localsourceid : -localsourceid), localDuration, localstatups, isMorph(), isMonsterRiding(), givemount));
            }
        ...
        if (isDs()) {
            ...
            applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), dsstat, false),
                    false);
        }
        if (isCombo()) {
            List<Pair<MapleBuffStat, Integer>> stat = Collections.singletonList(new Pair<MapleBuffStat, Integer>(
                    MapleBuffStat.COMBO, 1));
            applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, false),
                    false);
		...
		if (isOakBarrel() || isPirateMorph()) { //Kinda a hacky fix, but who cares..
			applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.removePlayerFromMap(applyto.getObjectId()), false);
			applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.spawnPlayerMapobject(applyto), false);
		}
	}
   ...
   public short getHpCon() {
	...
	public final boolean isGmBuff() {
        switch (sourceid) {
            ...
            case 9101008:
                return true;
            default:
                return Configuration.isBeginnerJob(sourceid / 10000) && sourceid % 10000 == 1005;
        }
    }
	...
	public boolean isResurrection() {
		return sourceid == 9101005 || sourceid == 2321006 || sourceid == SuperGM.RESURRECTION;
	}
