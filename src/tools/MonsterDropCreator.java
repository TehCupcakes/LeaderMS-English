...
public class MonsterDropCreator {
	...
	for (MapleData dataz : data.getChildren()) {
	    ...
	    if (dataz.getChildByPath("reward").getChildren().size() > 0) { // Fix for Monster without any reward causing SQL error
                ...
                StringBuilder append = sb.append("INSERT INTO ").append(monsterQueryData).append(" VALUES ");
		for (MapleData drop : dataz.getChildByPath("reward")) {
		    ...
		    if (first) {
			sb.append("(DEFAULT, ").append(idtoLog).append(", ").append(itemid).append(", ").append(rate).append(")");
			first = false;
		    } else {
			sb.append(", (DEFAULT, ").append(idtoLog).append(", ").append(itemid).append(", ").append(rate).append(")");
		    }
		...
		for (Pair<Integer, MobInfo> Pair_ : mobCache) {
			...
			if (Pair_.getRight().getBoss() > 0) {
			    rate /= 10;
                        ...
                        StringBuilder append = SQL.append("INSERT INTO ").append(monsterQueryData).append(" VALUES (DEFAULT, ").append(Pair_.getLeft()).append(", ").append(Pair.getLeft()).append(", ").append(rate).append(");");
			SQL.append("\n");
			break;
		    }
		}
	...
	time /= 1000;
        ...
        System.out.println("\nNow update your monsterdrops sucker");
    }
