...
public class BBSOperationHandler extends AbstractMaplePacketHandler {
	...
	public static void newBBSReply(MapleClient client, int localthreadid, String text) {
		...
		try {
			...
			ps = con.prepareStatement("INSERT INTO bbs_replies "
	...
	public static void newBBSThread(MapleClient client, String title, String text, int icon, boolean bNotice) {
		...
		try {
			...
			ps = con.prepareStatement("INSERT INTO bbs_threads "
				+ "(`postercid`, `name`, `timestamp`, `icon`, `startpost`, " + "`guildid`, `localthreadid`) "
				+ "VALUES(?, ?, ?, ?, ?, ?, ?)");
			ps.setInt(1, c.getId());
	...
	public static void displayThread(MapleClient client, int threadid, boolean bIsThreadIdLocal) {
		...
		try {
			...
			client.getSession().write(MaplePacketCreator.showThread(bIsThreadIdLocal ? threadid
				: threadRS.getInt("localthreadid"), threadRS, repliesRS));
			ps.close();
			if (ps2 != null)
				ps2.close();
		} catch (SQLException se) {
			...
			try {
				...
				log.warn("Problem deleting bbsthread", e);
			}
		}
	}
}
