	...
	public void start(MapleClient c, int npc, int quest) {
		try {
			...
			qs.start((byte)1, (byte)0, 0); 
                        ...
                        FilePrinter.printError(FilePrinter.QUEST + quest + ".txt", e);
			dispose(c);
		}
	}
		...
		if (qs != null) {
			try {
				qs.start(mode, type, selection);
                                ...
                                FilePrinter.printError(FilePrinter.QUEST + c.getQM().getQuest() + ".txt", e);
				dispose(c);
			}
		}
	}
	...
	public void end(MapleClient c, int npc, int quest) {
		try {
			...
			qs.end((byte) 1, (byte) 0, 0);
                        ...
                        FilePrinter.printError(FilePrinter.QUEST + npc + ".txt", e);
			dispose(c);
		}
	}
